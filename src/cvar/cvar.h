...
typedef struct ICvar {
  struct {}* IAppSystem_vftable;
  struct {
    CVarDllIdentifier_t (*AllocateDLLIdentifier)(struct ICvar* self);

    void (*RegisterConCommand)(struct ICvar* self, ConCommandBase* pCommandBase);
    void (*UnregisterConCommand)(struct ICvar* self, ConCommandBase* pCommandBase);
    void (*UnregisterConCommands)(struct ICvar* self, CVarDllIdentifier_t id);

    const char* (*GetCommandLineValue)(struct ICvar* self, const char* pVariableName);
    
    ConCommandBase* (*FindCommandBase)(struct ICvar* self, const char* name);
    const ConCommandBase* (*FindCommandBase_const)(const struct ICvar* self, const char* name);
    ...
    const ConCommand* (*FindCommand_const)(const struct ICvar* self, const char* name);
    ...
    void (*ConsoleColorPrintf)(struct ICvar* self, const SourceColor* clr, const char* pFormat, ...);
